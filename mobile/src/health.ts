...
// O Health Connect agrega Mi Fitness/Zepp (Mi Band/Xiaomi), Samsung Health e
// Google Fit — cobrindo as marcas mais comuns no Brasil. Módulo nativo: só
// funciona em dev build / build de produção (não no Expo Go).
...
/** Inicializa e pede as permissões de leitura. Retorna se ao menos uma foi concedida. */
...
async function safeRead(recordType: string, startTime: string, endTime: string): Promise<any[]> {
  try {
    ...
    return []; // o aparelho pode não ter esse tipo de dado
  }
}

/** Lê e agrega os últimos `days` dias em resumos diários (sono, FC repouso, HRV, passos). */
export async function readDailyHealth(days = 14): Promise<WearableDay[]> {
  ...
  for (const r of rhr) {
    if (r?.time && typeof r.beatsPerMinute === "number") bucket(localDay(r.time)).rhr.push(r.beatsPerMinute);
  }
  for (const r of hrv) {
    if (r?.time && typeof r.heartRateVariabilityMillis === "number") {
      bucket(localDay(r.time)).hrv.push(r.heartRateVariabilityMillis);
    }
  }
